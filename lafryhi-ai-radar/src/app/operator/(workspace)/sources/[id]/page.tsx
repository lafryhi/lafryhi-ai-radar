...
export default async function SourceDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ result?: string; error?: string; discovery?: string; runId?: string; candidate?: string }> }) {
  ...
  return <>
    ...
    <div className="metric-grid"><div className="metric-card"><span>Processed signals</span><strong>{statistics.processedArticles}</strong></div><div className="metric-card"><span>Verified</span><strong>{statistics.approved}</strong></div><div className="metric-card"><span>Rejected</span><strong>{statistics.rejected}</strong></div><div className="metric-card"><span>Last intelligence run</span><strong className="metric-small">{statistics.lastProcessingAt ? new Date(statistics.lastProcessingAt).toLocaleDateString() : "Never"}</strong></div></div>
    <section className="panel"><h2>Trusted Source controls</h2><p className="muted">State changes are explicit operator actions. Blocking also sets the trust level to Blocked.</p><SourceActions sourceDefinitionId={source.id} status={source.status} trust={source.trustLevel} /></section>
    <section className="panel"><h2>Controlled Signal discovery</h2><dl className="detail-grid"><div><dt>Configured feed</dt><dd>{safeFeed || "Not configured"}</dd></div><div><dt>Last signal scan</dt><dd>{lastDiscovery?.completedAt ? new Date(lastDiscovery.completedAt).toLocaleString() : "Never"}</dd></div><div><dt>Last status</dt><dd>{lastDiscovery?.status || "No run"}</dd></div><div><dt>Signals accepted</dt><dd>{lastDiscovery?.candidatesAccepted ?? 0}</dd></div></dl><DiscoverSourceButton sourceDefinitionId={source.id} eligible={eligible} hasFeed={Boolean(source.rssUrl)} /></section>
    <section><div className="section-heading"><h2>Discovered Signal Candidates</h2><span className="muted">Metadata only · Human Verification required</span></div><div className="stack">{candidates.map((candidate) => <article className="panel compact" key={candidate.id}><div className="split"><div><StatusBadge value={candidate.status} /><h3>{candidate.title}</h3></div><time>{new Date(candidate.discoveredAt).toLocaleString()}</time></div><p><a href={candidate.articleUrl} target="_blank" rel="noopener noreferrer">Open trusted signal evidence</a></p>{candidate.summary && <p className="muted">{candidate.summary}</p>}{candidate.status === "pending" ? <ProcessCandidateButton candidateId={candidate.id} sourceDefinitionId={source.id} /> : <p className="success">Processed into evidence record <code>{candidate.sourceRecordId}</code>.</p>}</article>)}{!candidates.length && <div className="panel empty">No Signal Candidates have been accepted.</div>}</div></section>
    <section className="panel"><h2>Trusted Source metadata</h2><dl className="detail-grid"><div><dt>Homepage</dt><dd><a href={source.homepage} target="_blank" rel="noopener noreferrer">Open homepage</a></dd></div><div><dt>RSS metadata</dt><dd>{safeFeed || "Not configured"}</dd></div><div><dt>Documentation</dt><dd>{source.documentationUrl ? <a href={source.documentationUrl} target="_blank" rel="noopener noreferrer">Open documentation</a> : "Not configured"}</dd></div><div><dt>Human Verification</dt><dd>Required</dd></div><div><dt>Created</dt><dd>{new Date(source.createdAt).toLocaleString()}</dd></div><div><dt>Updated</dt><dd>{new Date(source.updatedAt).toLocaleString()}</dd></div><div><dt>Trusted Source ID</dt><dd><code>{source.id}</code></dd></div></dl></section>
