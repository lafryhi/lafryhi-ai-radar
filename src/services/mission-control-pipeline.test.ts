...
describe("Mission Control demo pipeline", () => {
  ...
  it("keeps live collection separate from demo outputs and defers analysis", async () => { const liveRequest = MissionControlRequestSchema.parse({ mode: "live", period: { start: "2026-01-01T00:00:00.000Z", end: "2026-01-31T23:59:59.000Z" } }); const repository = { listSourceDefinitions: async () => [] } as unknown as RadarRepository; const result = await runMissionControl(liveRequest, () => new Date("2026-01-07T12:00:00.000Z"), repository); expect(result.mode).toBe("live"); expect(result.report).toBeNull(); expect(result.videoPackage).toBeNull(); expect(result.stages.slice(1).every((stage) => stage.errorCode === "LIVE_ANALYSIS_DEFERRED")).toBe(true); expect(result.items).toEqual([]); });
});
