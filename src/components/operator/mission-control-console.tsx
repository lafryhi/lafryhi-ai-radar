...
function ProgressRing({ progress, running, complete }: { progress: number; running: boolean; complete: boolean }) {
  const label = complete ? "Mission Complete" : running ? "Mission Running" : "Ready";
...
type DisplayStageStatus = "waiting" | "running" | "completed" | "failed";
...
const StageCard = memo(function StageCard({ definition, item, index, displayStatus }: { definition: typeof stages[number]; item?: PipelineStageResult; index: number; displayStatus: DisplayStageStatus }) {
  ...
  return <article className={`mission-stage-card mission-stage-${displayStatus}`} style={{ "--stage-delay": `${index * 55}ms` } as React.CSSProperties}>
    <div className="mission-stage-top"><span className="mission-stage-icon">{displayStatus === "completed" ? <Icon name="check" /> : displayStatus === "running" ? <span className="mission-stage-dot" /> : displayStatus === "failed" ? <span className="mission-stage-fail">!</span> : <span>{index + 1}</span>}</span><span className="mission-stage-status">{statusLabel}</span></div>
...
export function MissionControlConsole({ demoEnabled }: { demoEnabled: boolean }) {
  ...
  const run = async () => {
    ...
    try {
      const response = await fetch("/operator/mission-control/run", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ mode: demoEnabled ? "demo" : "live", period: { start: start.toISOString(), end: end.toISOString() } }) });
  ...
  const missionAnimating = running || Boolean(result && completedStageCount < stages.length);
  const complete = result?.status === "success" && completedStageCount >= stages.length;
  const progress = running ? 8 : result ? Math.round((completedStageCount / stages.length) * 100) : 0;
  const displayStatus = (index: number, item?: PipelineStageResult): DisplayStageStatus => {
    ...
    if (completedStageCount > index) return "completed";
    if (completedStageCount === index) return "running";
    return "waiting";
  };
  ...
  return <div className="mission-control-stack">
    <section className="mission-hero panel"><div className="mission-hero-copy"><span className="mission-kicker"><Icon name="spark" /> Weekly intelligence run</span><h2>Generate Weekly Intelligence</h2><p>One transparent workflow from trusted sources to editorial insight. Every conclusion remains traceable and human-controlled.</p><div className="mission-action-row"><button className="mission-primary-button" onClick={run} disabled={missionAnimating || !demoEnabled}><span>{missionAnimating ? "Mission running" : complete ? "Run again" : "Generate Weekly Intelligence"}</span>{missionAnimating ? <span className="mission-button-spinner" aria-hidden="true" /> : <Icon name="arrow" />}</button>{!demoEnabled && <span className="mission-disabled-note">Enable Demo Mode to run the prepared workflow.</span>}</div></div><ProgressRing progress={progress} running={missionAnimating} complete={complete} /></section>
    {!demoEnabled && <p className="notice" role="status">Demo Mode is disabled. Set <code>AI_RADAR_DEMO_MODE=true</code> to run the prepared workflow.</p>}
