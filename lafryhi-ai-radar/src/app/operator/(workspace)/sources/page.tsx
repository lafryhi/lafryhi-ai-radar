...
export default async function Sources({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">Trusted Signal foundation</p><h1>Trusted Source Governance</h1><p className="lede">Only enabled sources with Official, Verified, or Community trust can contribute signals to the Decision Intelligence pipeline.</p></div></div>
    ...
    <div className="source-grid">{result.sources.map((source) => <article className="panel compact" key={source.id}><div className="split"><div><p className="eyebrow">{source.publisher}</p><h2>{source.displayName}</h2></div><div className="badge-stack"><StatusBadge value={source.status} /><StatusBadge value={source.trustLevel} /></div></div><p><code>{source.canonicalDomain}</code></p><p className="muted">{source.category.replaceAll("_", " ")} · {source.language.toUpperCase()} · {source.country}</p><Link className="button-link" href={`/operator/sources/${source.id}`}>Manage Trusted Source</Link></article>)}{result.sources.length === 0 && <div className="panel empty"><h2>No Trusted Sources registered</h2><p>Register a trusted source before scanning for signals.</p></div>}</div>
