...
export default async function Runs({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  return <><p className="eyebrow">Intelligence evidence</p><h1>Intelligence Pipeline Runs</h1><p className="lede">Bounded operational history for Trusted Signal processing, Gemini Analysis, and reruns.</p>
    ...
    <h2>Signal discovery runs</h2><p className="lede">Metadata-only signal discovery never invokes Gemini or creates a Decision Brief.</p><div className="stack">{discoveryRuns.map((run) => <article className="panel compact" key={run.id}><div className="split"><div><StatusBadge value={run.status} /> <StatusBadge value={run.trigger} /></div><time>{new Date(run.startedAt).toLocaleString()}</time></div><code>{run.id}</code><dl className="detail-grid"><div><dt>Trusted Source ID</dt><dd><code>{run.sourceDefinitionId || "bounded scheduled set"}</code></dd></div><div><dt>Sources</dt><dd>{run.sourcesConsidered}</dd></div><div><dt>Signals examined</dt><dd>{run.itemsExamined}</dd></div><div><dt>Accepted</dt><dd>{run.candidatesAccepted}</dd></div><div><dt>Duplicates</dt><dd>{run.duplicates}</dd></div><div><dt>Skipped / validation</dt><dd>{run.skippedItems} / {run.validationFailures}</dd></div></dl></article>)}{!discoveryRuns.length && <div className="panel empty">No signal discovery runs recorded.</div>}</div>
  </>;
}
