...
function ProgressRing({ progress, running, complete, live }: { progress: number; running: boolean; complete: boolean; live: boolean }) {
  const label = complete ? "Mission Complete" : live && !running && progress === 100 ? "Collection Complete" : running ? "Mission Running" : "Ready";
...
type DisplayStageStatus = "waiting" | "running" | "completed" | "failed" | "deferred";
...
const StageCard = memo(function StageCard({ definition, item, index, displayStatus }: { definition: typeof stages[number]; item?: PipelineStageResult; index: number; displayStatus: DisplayStageStatus }) {
  ...
  return <article className={`mission-stage-card mission-stage-${displayStatus}`} style={{ "--stage-delay": `${index * 55}ms` } as React.CSSProperties}>
    <div className="mission-stage-top"><span className="mission-stage-icon">{displayStatus === "completed" ? <Icon name="check" /> : displayStatus === "running" ? <span className="mission-stage-dot" /> : displayStatus === "failed" ? <span className="mission-stage-fail">!</span> : displayStatus === "deferred" ? <span>—</span> : <span>{index + 1}</span>}</span><span className="mission-stage-status">{statusLabel}</span></div>
...
export function MissionControlConsole({ demoEnabled }: { demoEnabled: boolean }) {
  ...
  const activeStageCount = liveResult ? 1 : stages.length;
  ...
  const progress = running ? 8 : result ? liveResult ? (completedStageCount >= 1 ? 100 : 8) : Math.round((completedStageCount / stages.length) * 100) : 0;
  const displayStatus = (index: number, item?: PipelineStageResult): DisplayStageStatus => {
    ...
    if (liveResult && index > 0) return "deferred";
  ...
  return <div className="mission-control-stack">
    ...
    {liveResult && !missionAnimating && <section className="mission-live-notice" role="status"><strong>Live collection complete</strong><span>Analysis, ranking, editorial review, report, and video remain deferred to Sprint 3.2.</span></section>}
