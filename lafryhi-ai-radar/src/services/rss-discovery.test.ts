...
import { handleOperatorRssDiscovery, handleScheduledRssDiscovery } from "./rss-route-handlers";
...
describe("controlled RSS discovery", () => {
  ...
  afterEach(() => { vi.restoreAllMocks(); delete process.env.OPERATOR_ACCESS_TOKEN; delete process.env.RSS_SCHEDULER_JOB_NAME; delete process.env.RSS_SCHEDULER_SECRET; });
  ...
  it("requires Cloud Scheduler headers in addition to private Cloud Run IAM", async () => {
    ...
    expect(unauthorized.status).toBe(401);
    const authorized = await handleScheduledRssDiscovery(new NextRequest("https://example.test/api/internal/rss/scheduled", { method: "POST", headers: { "x-cloudscheduler": "true", "x-cloudscheduler-jobname": "projects/test/locations/us-central1/jobs/lafryhi-ai-radar-rss-discovery", "x-internal-scheduler-secret": "phase-six-scheduler-secret" } }), repository);
    expect(authorized.status).toBe(200);
  });
