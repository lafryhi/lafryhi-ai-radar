...
export function ReviewQueue({ entries, empty = "No reviews match these filters." }: { entries: ReviewQueueEntry[]; empty?: string }) {
  if (!entries.length) return <div className="panel empty"><h2>Nothing to review</h2><p>{empty}</p></div>;
  return <div className="review-table-wrap"><table className="review-table">
    <thead><tr><th>Analysis</th><th>Scores</th><th>Processing</th><th>State</th><th><span className="sr-only">Open</span></th></tr></thead>
    <tbody>{entries.map((entry) => <tr key={entry.analysis.id}>
      ...
      <td><Link className="button-link" href={`/operator/review/${entry.analysis.id}`}>Inspect</Link></td>
    </tr>)}</tbody>
  </table></div>;
}
